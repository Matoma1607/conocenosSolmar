...
function initializeAnimations() {
    ...
    const observer = new IntersectionObserver(function(entries) {
        entries.forEach(entry => {
            if (entry.isIntersecting) {
                ...
                const children = entry.target.querySelectorAll('.feature-card, .space-card, .branch-card');
    ...
    document.querySelectorAll('.feature-card, .space-card, .branch-card').forEach(card => {
...
function initializeScrollEffects() {
    ...
    function updateScrollEffects() {
        ...
        if (scrolled > 100) {
            ...
            header.style.background = 'rgba(131, 153, 150, 0.95)';
            header.style.backdropFilter = 'blur(10px)';
        } else {
            header.classList.remove('scrolled');
            header.style.background = '#839996';
...
function getBranchDetailContent(branchName) {
    const branchDetails = {
        'Sucursal Centro': {
            description: 'Nuestra sucursal principal ubicada en el corazón de la ciudad, equipada con la más avanzada tecnología para exámenes visuales.',
            services: [
        ...
        'Sucursal Norte': {
            description: 'Ubicada en el moderno Shopping Norte, ofrecemos la mayor variedad de monturas y lentes de sol de la región.',
            services: [
        ...
        'Sucursal Sur': {
            description: 'En el tranquilo Barrio Jardín, brindamos atención personalizada en un ambiente relajado y familiar.',
            services: [
                'Exámenes visuales',
                'Monturas exclusivas',
        ...
        'Sucursal Este': {
            description: 'Próxima apertura en la zona de mayor crecimiento de la ciudad. Será nuestra sucursal más moderna y tecnológica.',
            services: [
...
function contactBranch(branchName) {
    ...
    if (phone && phone !== 'Próximamente') {
        ...
        const whatsappMessage = encodeURIComponent(`Hola, me interesa obtener información sobre los servicios de ${branchName} - Óptica Solmar`);
        ...
        alert('Esta sucursal abrirá próximamente. Manténgase atento a nuestras redes sociales para más información.');
    }
}
...
function handleError(error) {
    ...
    showNotification('Ha ocurrido un error. Por favor, inténtelo nuevamente.', 'danger');
}

// Event listeners adicionales
window.addEventListener('resize', function() {
    // Reajustar elementos responsive
    const cards = document.querySelectorAll('.feature-card, .space-card, .branch-card');
    cards.forEach(card => {
...
if (isMobileDevice()) {
    document.addEventListener('DOMContentLoaded', function() {
        ...
        const hoverElements = document.querySelectorAll('.feature-card, .space-card, .branch-card');
